...
def surrender_match(current_user, match_id):
    try:
        # Verificar que el usuario está en la partida y que es una partida activa
        matches = dao_matches.getPlayerMatches(current_user['id'])
        print(matches)
        if not matches:
            return jsonify({"error": "No estás en ninguna partida activa"}), 403
            
        current_match = None
        for m in matches:
            if m['id'] == match_id and m['status'] == 'active':
                current_match = m
                break
        ...
        if not current_match:
            return jsonify({
                ...
                "debug_info": {
                    "match_id": match_id,
                    "user_matches": [m['id'] for m in matches],
                    "current_user": current_user['id']
                }
            }), 403
        ...
        if not success:
            return jsonify({
                "error": "Error al rendirse",
                "possible_reasons": [
                    "La partida ya estaba terminada",
                    "No eras un jugador de esta partida",
        ...
        return jsonify({
            "error": f"Error al rendirse: {str(e)}",
