...
class DAOMatches:
    ...
    def endMatch(self, match_id, surrendering_player_id):
        ...
        try:
            # Obtener información de la partida
            query = "SELECT player1_id, player2_id, status FROM partidas WHERE id = %s"
            cursor.execute(query, (match_id,))
            match = cursor.fetchone()
            ...
            # Verificar que la partida no esté ya terminada
            ...
            elif surrendering_player_id == match['player2_id']:
                winner_id = match['player1_id']
            else:
                return False  # El jugador no está en esta partida
